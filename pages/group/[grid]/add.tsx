...
const Home: FC = () => {
  ...
  return (
    ...
    >
      <div className="my-2 h-full w-full max-w-3xl flex flex-col justify-start">
        <form onSubmit={handleSubmit(onSubmit)}>
          <textarea
            autoComplete="off"
            {...register('body')}
            className="m-2 p-2 max-w-2xl w-full h-52 border-2 border-gray-400 rounded-xl focus:outline-none text-gray-700"
          />

          {file && <div className="m-2">{file.name}</div>}
          <label htmlFor="file-upload" className="flex mb-4">
            <input
              ...
              multiple={false}
            />
            <div className="m-2 bg-gray-700 text-gray-50 py-2 px-4 rounded-lg text-sm cursor-pointer inline-block">
              Add Code
            </div>
          </label>
          ...
          <button
            type="submit"
            className="mx-2 px-4 py-1.5 border bg-gray-700 rounded-md text-gray-100"
          >
            Post
          </button>
        </form>
