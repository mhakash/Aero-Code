...
export const createNewGroup = async (
  ...
  try {
    ...
    const promise = await userCollection.updateMany(
      { _id: { $in: id_array } },
      { $push: { groups: { _id: group_id.insertedId, name: groupname } } },
    );
...
export const getDiscussionByGroupID = async (
  ...
  try {
    ...
    const nonMemberfriends: { _id: string; name: string }[] =
      user.friends?.filter((item) => !memberids.includes(item._id)) ?? [];

    const data = {
      posts: posts,
      group: group,
      friends: nonMemberfriends as { _id: string; name: string }[],
    };
    return data;
  } catch (err) {
...
export const addGroupMember = async (
  ...
  try {
    ...
    const promise = await groupCollection.updateOne(
      { _id: new ObjectId(grid) },
      { $push: { members: { _id: _id, name: name, role: 0 } } },
    );
    ...
    const promise2 = await userCollection.updateOne(
      { _id: _id },
      { $push: { groups: { _id: group._id, name: group.name } } },
    );
